...
// Pulse measurement
static volatile int64_t last_rise_us[CHANNEL_COUNT] = {0, 0};
static volatile int64_t last_edge_us[CHANNEL_COUNT] = {0, 0};
static volatile int64_t pulse_width_us[CHANNEL_COUNT] = {0, 0};
static volatile int64_t period_us[CHANNEL_COUNT] = {0, 0};

static void IRAM_ATTR signal_isr(void *arg)
{
  int ch = (int)arg;
  int level = gpio_get_level(ch == 0 ? RECEIVER_CH1 : RECEIVER_CH2);
...
{
  gpio_config_t io_conf = {};
  io_conf.pin_bit_mask = (1ULL << RECEIVER_CH1) | (1ULL << RECEIVER_CH2);
  ...
  printf("drive_system initialized on GPIO %d (CH1) and GPIO %d (CH2)\n", RECEIVER_CH1, RECEIVER_CH2);
...
void drive_system_loop(WS2812FX *fx)
{
  ...
  float throttle_scaled = (throttle * 2.0f) - 1.0f; // -1.0 (reverse) to +1.0 (forward)
  float steering_scaled = (steering * 2.0f) - 1.0f; // -1.0 (right) to +1.0 (left)
  ...
  // Debug output
  // printf("VOLTAGE: %.2f | Throttle: %.2f | Steering: %.2f | Left: %d%% | Right: %d%%\n",
  //        input_voltage, throttle_scaled, steering_scaled, left_speed, right_speed);
